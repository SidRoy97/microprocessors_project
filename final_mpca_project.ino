...
void loop()
{
  if (should_reset) {
    int pos;
  for (pos = 180; pos >= 90; pos --) { // goes from 0 degrees to 180 degrees
...
if (key == '#') {
  ...
  {  
   ...
   sprintf(buf, "Someone entered wrong password!!\n");
    Serial.print(buf); 
  }
  password.reset();
} else if (key == '*') {
...
if (temp_n < TEMP_MAX) {
  ...
  if (avg_temp >= 80.0) {
    //sprintf(buf, "Temperature is very high in your home!!\n"/*, (int)avg_temp*/);
    //Serial.print(buf);
      //digitalWrite(12,HIGH);
  }
}/*
...
float farh = (cel*9)/5 + 32;
 ...
 int analogSensor = analogRead(smoke );
  ...
  if(analogSensor > 350){
    digitalWrite(12, HIGH);
    sprintf(buf, "There is smoke is your house\n");
    Serial.print(buf);
    //Serial.print("Pin A0: ");
    //Serial.println(analogSensor);
    //delay(100);
